...
//        ".then(data => {FS.writeFile('" + targetFile + "', new Uint8Array(data)); FS.writeFile('" + targetFile + ".finish', 'finish'); });}";
  ...
  public static void fetchx(String url, String tarDir, CallbackItem cb)
  {
    long id = NativeCallback.registerCallback(cb);
    int idx = url.lastIndexOf("/");
    String targetFile = tarDir + "/" + url.substring(idx);
...
//    sb.append("  }).catch((error) => { Module.ccall('native_callback', null, ['string', 'number'], [null, ").append(id).append("]); });");
    sb.append("  });");
    sb.append("};");
    System.out.println("org.mini.util.wasm.Downloader.fetch(pre)");
    WasmUtil.executeJS(sb.toString(),true,false);
    System.out.println("org.mini.util.wasm.Downloader.fetch(post)");
  }
}
